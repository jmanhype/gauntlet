...
def versions() -> ArtifactVersions:
    return ArtifactVersions(*(["1"] * 6), sha256_digest(b"schemas"), sha256_digest(b"environment"))
...
def config(tmp_path: Path) -> ResolvedConfig:
    ...
    return resolve_config(profile, {}, versions())
...
def scene(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    root = tmp_path / "gauntlet-data"; monkeypatch.setenv("GAUNTLET_DATA_ROOT", str(root)); population = build_synthetic_population(7, POLICY)
    return root, population, build_split_manifest(POLICY, population), config(tmp_path)
...
def test_synthetic_population_is_deterministic_registered_and_local(scene) -> None:
    ...
    assert population.mapping()["promotable_venue_evidence"] is False and population.mapping()["evidence_class"] == "SYNTHETIC_FIXTURE"
    assert [row.row_id for row in population.bars] == sorted(row.row_id for row in population.bars)
    ...
    for module in ((Path(__file__).parents[2] / "src" / "gauntlet" / "judge" / name) for name in ("synthetic.py", "walk_forward.py")):
        ...
        assert not any(isinstance(node, (ast.Import, ast.ImportFrom)) and any(alias.name.split(".")[0] in {"socket", "http", "urllib", "requests"} for alias in node.names) for node in ast.walk(tree))
        assert "bitquery" not in module.read_text(encoding="utf-8").casefold()
...
def test_split_manifest_records_strict_boundaries_labels_and_locks(scene) -> None:
    ...
    assert value["fold_order"] == [1, 2] and len(value["folds"]) == 2
    assert value["policy"]["horizon_bars"] == value["policy"]["target_horizon_bars"] == POLICY.target_horizon_bars
    assert value["policy"]["lookback_bars"] == POLICY.lookback_bars and value["policy"]["normalization_window_bars"] == POLICY.normalization_window_bars
    assert value["policy"]["embargo_bars"] == POLICY.embargo_bars and value["policy"]["purge_bars"] == POLICY.purge_bars
    ...
    for fold in manifest.folds:
        ...
        train_end = datetime.fromisoformat(fold.train_end_utc.replace("Z", "+00:00")); validation_start = datetime.fromisoformat(fold.validation_start_utc.replace("Z", "+00:00"))
        validation_end = datetime.fromisoformat(fold.validation_end_utc.replace("Z", "+00:00")); test_start = datetime.fromisoformat(fold.test_start_utc.replace("Z", "+00:00"))
        ...
        assert not prior_tests & set(fold.test_membership)
        prior_tests.update(fold.test_membership)
...
def test_real_multi_fold_evaluation_selects_on_validation_and_replays_rows(scene) -> None:
    ...
    assert result.status == "OK" and result.run_report["dependency_evaluation"]["status"] == "OK" and result.run_report["selected_fingerprints"] == [buy.fingerprint, buy.fingerprint]
    assert result.run_report["factory_ranking"] == {"admissible": False, "ranking_score_is_judge_evidence": False, "entry_condition": "selected_fingerprint_locked"}
    ...
    for prediction in result.predictions:
        assert prediction.venue == "solana_dex" and prediction.token and prediction.intended_action in {"BUY", "FLAT"}
        assert prediction.prediction_basis == prediction.label_basis == "MODELED" and prediction.selected_fingerprint == buy.fingerprint
    ...
    for trade in result.trades:
        assert trade.label_completed_at_utc and trade.execution_basis == trade.outcome_basis == "MODELED" and trade.selection_lock_hash
        assert datetime.fromisoformat(trade.entry_timestamp_utc.replace("Z", "+00:00")) < datetime.fromisoformat(trade.exit_timestamp_utc.replace("Z", "+00:00"))
    ...
    assert json.loads(result.artifact_paths["run_report.json"].read_bytes())["evaluation_hash"] == result.run_report["evaluation_hash"]
    assert (root / "ledger" / "events.jsonl").read_bytes().count(b"\n") == 1 and verify_chains(root).valid
    replay = evaluate_walk_forward(WalkForwardRequest(*arguments))
    assert replay.run_hash == result.run_hash and replay.replayed and (root / "ledger" / "events.jsonl").read_bytes().count(b"\n") == 1
...
def test_fail_closed_rejections_and_missing_dependency_blocks(scene) -> None:
    ...
    assert blocked.status == "BLOCKED" and blocked.run_report["blocked_reason"] == "BLOCKED"
    assert blocked.predictions == () and blocked.trades == () and blocked.run_report["execution"]["imputed_fill"] is False
    assert any(finding["code"] == "DEPENDENCY_MISSING" for finding in blocked.run_report["dependency_evaluation"]["findings"])
